...
public class TogglePartecipazioneProdottoListaServlet extends HttpServlet {
	...
	private void doJob(HttpServletRequest request, HttpServletResponse response)
		...
		int idProdotto=Integer.parseInt(request.getParameter("idProdottoLista"));
		boolean partecipoIo=Boolean.parseBoolean(request.getParameter("partecipoIo"));
		int idUtente=((Utente)request.getSession().getAttribute("utente")).getId();
		
		if(partecipoIo){
			dataAccess.remPartecipazioneProdotto(idUtente, idProdotto);
		}else{
			dataAccess.addPartecipazioneProdotto(idUtente, idProdotto);
		}
